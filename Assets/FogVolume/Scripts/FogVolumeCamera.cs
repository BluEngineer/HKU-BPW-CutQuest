    ...
    public void RenderDepth()
    {
        if (_FogVolumeRenderer.GenerateDepth)
        {
            ...
            if (SceneCamera.stereoEnabled)
            {
                ...
                {
                    Vector3 eyePos = SceneCamera.transform.parent.TransformPoint(InputTracking.GetLocalPosition(XRNode.LeftEye));
                ...
                {
                    Vector3 eyePos = SceneCamera.transform.parent.TransformPoint(InputTracking.GetLocalPosition(XRNode.RightEye));
                    Quaternion eyeRot = SceneCamera.transform.rotation;
